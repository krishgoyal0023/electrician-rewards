...
import { ShieldCheck, Download, CheckCircle2, Clock, Users, QrCode, RefreshCw } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const fetchAdminData = async () => {
    ...
    // 1. Fetch Redemption Requests with Electrician & Reward details
    const { data: redemptions } = await supabase
      .from('redemption_requests')
      .select('id, points_spent, status, created_at, electricians(name, phone), rewards(title)')
      .order('created_at', { ascending: false });

    setRequests(redemptions || []);

    // 2. Fetch High-level Analytics
    ...
    <main className="min-h-screen bg-slate-950 text-white p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
            <div className="flex items-center gap-2 text-slate-400 text-xs mb-1">
              <Users className="h-4 w-4 text-amber-400" /> Electricians
            </div>
            <p className="text-2xl font-bold">{metrics.users}</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
            <div className="flex items-center gap-2 text-slate-400 text-xs mb-1">
              <QrCode className="h-4 w-4 text-amber-400" /> Total Coupons
            </div>
            <p className="text-2xl font-bold">{metrics.coupons}</p>
          </div>
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
            <div className="flex items-center gap-2 text-slate-400 text-xs mb-1">
              <Clock className="h-4 w-4 text-amber-400" /> Pending Payouts
            </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
          <div className="p-4 border-b border-slate-800 font-bold text-sm">Payout & Reward Requests</div>
                ...
                <tbody className="divide-y divide-slate-800">
                  {requests.map((req) => (
                    <tr key={req.id} className="hover:bg-slate-800/30 transition">
                      ...
                      <td className="p-3 font-mono">{req.electricians?.phone || 'N/A'}</td>
